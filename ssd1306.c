...
// Buffer do display (1 byte por linha)
...
void ssd1306_send_command(ssd1306_t *ssd, uint8_t command) {
    ...
    i2c_write_blocking(ssd->i2c, ssd->address, data, 2, false);
}

// Inicializa o display
void ssd1306_init(ssd1306_t *ssd, uint8_t width, uint8_t height, uint8_t addr, i2c_inst_t *i2c) {
    ssd->i2c = i2c;
    ssd->address = addr;
    ssd->width = width;
    ssd->height = height;
    ...
    i2c_write_blocking(i2c, addr, init_seq, sizeof(init_seq), false);

    ssd1306_fill(ssd, false);
    ssd1306_send_data(ssd);
}

// Atualiza a tela enviando os dados do buffer
void ssd1306_send_data(ssd1306_t *ssd) {
    uint8_t data[SSD1306_WIDTH + 1];
    data[0] = 0x40;  // Comando de início de dados

    for (int i = 0; i < (SSD1306_HEIGHT / 8); i++) {
        memcpy(&data[1], &buffer[i * SSD1306_WIDTH], SSD1306_WIDTH);
        i2c_write_blocking(ssd->i2c, ssd->address, data, SSD1306_WIDTH + 1, false);
    }
}

// Preenche a tela com preto ou branco
void ssd1306_fill(ssd1306_t *ssd, bool color) {
    memset(buffer, color ? 0xFF : 0x00, sizeof(buffer));
}

// Desenha um retângulo 8x8
void ssd1306_rect(ssd1306_t *ssd, uint8_t x, uint8_t y, uint8_t w, uint8_t h, bool fill, bool color) {
    for (uint8_t i = 0; i < h; i++) {
        for (uint8_t j = 0; j < w; j++) {
            int index = x + j + ((y + i) / 8) * SSD1306_WIDTH;
            if (fill) {
                buffer[index] |= (1 << ((y + i) % 8));
            } else {
                buffer[index] &= ~(1 << ((y + i) % 8));
            }
        }
